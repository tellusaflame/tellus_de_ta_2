...
def buy_item(customer: Customer, vm: VendingMachine):
    ...
    while user_bill != "":
        ...
        user_bill = input(
            "Внесите деньги, или оставьте поле пустым для перехода к выбору товара:"
        )
    ...
    while not available or item is None:
        ...
        if item is not None and not available:
            ...
            user_bill = input(
                "Не хватает средств для покупки товара. Внесите деньги, или выберите другой товар:"
            )
            while user_bill != "":
                customer.credit = customer.add_cash(
                    Money(bill=int(user_bill), amount=1)
                )
    ...
    while True:
        ...
        if choice == "1":
            if vm.items:
                ...
                print(
                    "В машине нет товара для покупки. Уже вызвали хозяина, он разберется."
                )
